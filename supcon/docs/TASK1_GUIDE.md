...
### 3.3 标定观察位与安全位（config.yaml）
1. 用网页控制面板把臂摆到「能清楚拍全 3 盏灯」的位置 → `python scripts/02_record_pose.py --where` 拿到数值；
2. 填入 `config.yaml` 的 `arm.observe_pose`；
3. 再摆一个「远离面板、绝对安全」的休息位 → 填 `arm.safe_pose`。
...
| 3 | 开关1 重复步骤 1、2（--switch 1） | |
| 4 | 拨动开关：摆到拨杆**起始端** | `python scripts/02_record_pose.py --switch 2 --key flick_start_pose` |
| 5 | 摆到拨杆**拨完后的末端** | `python scripts/02_record_pose.py --switch 2 --key flick_end_pose` |
| 6 | 拨动开关正上方 | `python scripts/02_record_pose.py --switch 2 --key approach_pose` |

要点：
- 面板文件里 3 个开关的类型已预设为 button/button/toggle，若实际排列不同，直接改 `panel.json`；每盏灯还必须显式填写 `switch_id`，**不能依赖灯与开关的左右顺序相同**；
